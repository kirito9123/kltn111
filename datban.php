	...
	if (isset($_GET['ajax']) && $_GET['ajax'] === 'ban') {
		...
		try {
			...
			$rs = $listban->show_ban($id_phong, $id_loaiban);
			if (!$rs instanceof mysqli_result) {
				_json_end(['success' => false, 'message' => 'Kiểu trả về không hỗ trợ']);
			}
			...
			while ($r = $rs->fetch_assoc()) {
				$tables[] = [
					'id_ban'    => (int)$r['id_ban'],
					'tenban'    => $r['tenban'],
					// Bàn available nếu trangthai = 0 hoặc NULL
					'available' => !isset($r['trangthai']) || (int)$r['trangthai'] === 0
				];
			}

			_json_end(['success' => true, 'tables' => $tables]);
		} catch (Throwable $e) {
			_json_end(['success' => false, 'message' => 'Lỗi server khi load bàn']);
		}
	}
	?>
	...
	<head>
		...
		<link rel="stylesheet" href="scss/datban/datban.scss">
	</head>
	<body>
							...
							if ($show_loaiphong instanceof mysqli_result && $show_loaiphong->num_rows > 0) {
								...
								while ($row = $show_loaiphong->fetch_assoc()) {
									...
									foreach ($icons as $key => $value) {
										if (strpos($tenLoaiPhong, $key) !== false) { $icon = $value; break; }
									}
	...
	if (!selectedLoaiPhong) { alert('Vui lòng chọn loại phòng!'); return; }
	...
	fetch(`datban.php?ajax=ban&id_phong=${idPhong}&id_loaiban=${idLoaiBan}`)
		.then(async (response) => {
		const text = await response.text();
		try {
			const data = JSON.parse(text);
		...
		if (data.success && Array.isArray(data.tables) && data.tables.length > 0) {
			...
			banGrid.innerHTML += `
				...
				<div class="table-status">${tbl.available ? 'Trống' : 'Đã đặt'}</div>
				</label>`;
			});
	...
	wrap.addEventListener('click', (e) => { if (e.target.id === 'choiceOverlay') hideChoiceOverlay(); });
