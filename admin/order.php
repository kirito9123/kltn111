...
if (isset($_GET['ban_ids'])) {
    ...
    foreach ($tokens as $tk) {
        if ($tk === '') continue;
        if (ctype_digit($tk)) $idNums[] = (int)$tk; else $names[] = $tk;
    }
    if (!empty($names)) {
        $escNames = array_map(function($s) use ($db) { return "'" . $db->link->real_escape_string($s) . "'"; }, $names);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $now   = date('H:i');
    $ban_labels = [];
    foreach ($banList as $b) { if (!empty($b['tenban'])) $ban_labels[] = $b['tenban']; }
...
<style>
    ...
    .grid_10 { width: 100% !important; margin: 0 !important; max-width: 100%; }
    .pos-wrap { display: flex; gap: 15px; height: calc(100vh - 60px); background: #f4f6f9; padding: 10px; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; overflow: hidden; }
    
    /* CỘT TRÁI: MENU */
    .pos-left { flex: 7; display: flex; flex-direction: column; overflow: hidden; }
    .filter-row-search, .filter-row-cat {
        display: flex;
        align-items: center;
        gap: 10px;
        padding: 10px;
    ...
    .filter-row-cat {
        margin-top: 10px;
        padding-top: 5px; /* Giảm padding cho hàng category */
        padding-bottom: 5px;
    }
    
    .search-input { 
        flex: 1; /* Cho input chiếm hết khoảng trống còn lại */
        padding: 8px 15px; border: 1px solid #ddd; border-radius: 20px; outline: none; 
    }
    
    .cat-list { 
        display: flex; 
        gap: 8px; 
        overflow-x: auto; /* Giữ khả năng cuộn ngang */
        max-width: 100%;
        padding: 5px 0; /* Cho khoảng trống cuộn */
    }
    .cat-item { white-space: nowrap; padding: 6px 12px; border-radius: 15px; background: #f1f1f1; color: #333; text-decoration: none; font-size: 13px; border: 1px solid transparent; }
    .cat-item:hover, .cat-item.active { background: #0d6efd; color: #fff; border-color: #0d6efd; }
    
    .menu-scroll { flex: 1; overflow-y: auto; padding-right: 5px; padding-bottom: 50px; }
    .section-head { font-weight: bold; margin: 15px 0 10px; border-left: 4px solid #ffc107; padding-left: 10px; color: #444; text-transform: uppercase; }
    
    .pos-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(140px, 1fr)); gap: 12px; }
    .pos-card { background: #fff; border-radius: 10px; overflow: hidden; border: 1px solid #eee; cursor: pointer; transition: 0.2s; position: relative; }
    .pos-card:hover { transform: translateY(-3px); box-shadow: 0 5px 15px rgba(0,0,0,0.1); border-color: #0d6efd; }
    .card-img { height: 100px; width: 100%; object-fit: cover; background: #eee; }
    .card-info { padding: 10px; flex: 1; display: flex; flex-direction: column; justify-content: space-between; }
    .card-name { font-weight: 600; font-size: 13px; margin-bottom: 4px; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; height: 36px; }
    .card-price { color: #d63384; font-weight: bold; font-size: 14px; }
    .badge-combo { position: absolute; top: 6px; right: 6px; background: #ff5722; color: #fff; font-size: 10px; padding: 2px 6px; border-radius: 4px; }

    /* CỘT PHẢI: BILL */
    .pos-right { flex: 3; background: #fff; border-radius: 10px; display: flex; flex-direction: column; border: 1px solid #ddd; box-shadow: -2px 0 10px rgba(0,0,0,0.05); }
    .bill-head { padding: 15px; background: #f8f9fa; border-bottom: 1px solid #eee; display: flex; justify-content: space-between; align-items: center; }
    .bill-body { flex: 1; overflow-y: auto; padding: 10px; background: #fff; }
    .bill-foot { padding: 15px; border-top: 1px solid #eee; background: #fdfdfd; }
    
    .cart-item { display: flex; justify-content: space-between; align-items: center; padding: 8px 0; border-bottom: 1px dashed #eee; font-size: 13px; }
    .c-info { flex: 1; padding-right: 5px; }
    .c-name { font-weight: 600; display: block; }
    .c-qty { display: flex; align-items: center; gap: 5px; background: #f1f1f1; border-radius: 15px; padding: 2px; }
    .btn-qty { width: 22px; height: 22px; border: none; background: #fff; border-radius: 50%; cursor: pointer; font-weight: bold; box-shadow: 0 1px 2px rgba(0,0,0,0.1); }
    .c-total { font-weight: bold; min-width: 60px; text-align: right; }
    
    .total-row { display: flex; justify-content: space-between; font-size: 18px; font-weight: bold; margin-bottom: 15px; color: #d63384; }
    .btn-pay { width: 100%; padding: 12px; background: #198754; color: #fff; border: none; border-radius: 6px; font-size: 16px; font-weight: bold; cursor: pointer; text-transform: uppercase; }
    .btn-pay:disabled { background: #ccc; cursor: not-allowed; }

    /* Pagination */
    .pos-pagination { display: flex; justify-content: center; gap: 5px; margin-top: 20px; }
    .p-link { padding: 6px 12px; border: 1px solid #dee2e6; background: #fff; color: #0d6efd; text-decoration: none; border-radius: 4px; font-size: 13px; }
    .p-link.active { background: #0d6efd; color: #fff; }
</style>
